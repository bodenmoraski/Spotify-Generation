...
- Do not commit `.env`. Secrets belong in GitHub Actions secrets and in the Cloud Agents Secrets tab, never in the repo. Primary LLM secret is `DEEPSEEK_API_KEY`; `GEMINI_API_KEY` is fallback only.
- Do not implement X/Twitter ingestion.

The pipeline is implemented on `main`. Default models are DeepSeek V4 Flash (triage) and DeepSeek V4 Pro (editorial).
